...
if(eff==1)
{
    $('#screen, #modal').show();
    		...
    		document.getElementById("m-lat-lngs").innerHTML=JSON.stringify(this._plan.options.waypoints, null, 2);
    		var saveWaypoints=this._plan.getWaypoints();
    		$('#screen, #modal button').click(function(){
    			if(document.getElementById('done').checked) {
  m[mNum] = new L.Routing.control({
      ...
      styles: [{color: 'green', opacity: 1, weight: 5}]
          ...
          addWaypoints: true,
           router: L.Routing.mapbox('pk.eyJ1IjoiaW50ZXJuM3QiLCJhIjoiY2l6MWJyMWoyMDU3YjJ4bzRiZm5zeGN5eCJ9.T8dnG6_AgfPo5FCh49MpwQ')

               }).addTo(map);
               mNum++;
					
					this._plan.spliceWaypoints(0, 15);
}
...
if(document.getElementById('cancelled').checked) {
  m[mNum] = new L.Routing.control({
    ...
    $('#screen, #modal').hide();
					...
					//alert (JSON.stringify(this._plan.getWaypoints(), null, 2));

					//alert (JSON.stringify(this._plan.options.waypoints, null, 2));
...
var icon1=createIcons('images/blue.png', 30,30);
...
amina[change-1]=null;
	amina[change-1]=marker.getLatLng();
	 
	//alert(amina[m]+" "+m+" "+marker.getLatLng());
	//alert (routeA[rA]);
...
//update ith element of table
				    	$('#markerLatLng').val(e.latlng);
				    	
				    	MQ.geocode().reverse(e.latlng).on('success', function(e) {
      					$('#map-location').val(' ');
      					$('#map-location').val(geocode.describeLocation(e.result.best));
      					replaceNewlines();
